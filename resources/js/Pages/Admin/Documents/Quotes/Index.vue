...
async function remove(id){
  if(await confirmDialog('ลบเอกสารนี้?')){
    router.delete(`/admin/documents/quotes/${id}`, { onFinish: () => router.reload() })
  }
}
...
async function doApproval(commentArg){
  ...
  const url = approvalAction.value==='submit' ? `/admin/documents/quotes/${id}/submit` :
              approvalAction.value==='approve' ? `/admin/documents/quotes/${id}/approve` :
              approvalAction.value==='lock' ? `/admin/documents/quotes/${id}/lock` :
