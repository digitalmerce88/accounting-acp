<template>
  <AdminLayout>
    <div class="flex items-center justify-between">
      <h1 class="text-xl font-semibold">Quote {{ item.number || item.id }}</h1>
        <div class="flex gap-2">
          <a :href="`/admin/documents/quotes/${item.id}/history`" class="px-3 py-1 border rounded">ประวัติ</a>
          <a :href="`/admin/documents/quotes/${item.id}/pdf`" target="_blank" rel="noopener" class="px-3 py-1 border rounded">ดูใบเสนอราคา</a>
          <a :href="`/admin/documents/quotes/${item.id}/pdf?dl=1`" class="px-3 py-1 border rounded">ดาวน์โหลด</a>
          <a v-if="!['approved','locked'].includes(item.approval_status)" :href="`/admin/documents/quotes/${item.id}/edit`" class="px-3 py-1 border rounded">แก้ไข</a>
...
async function onDelete(){
  if (await confirmDialog('ลบเอกสารนี้?')) {
    router.delete(`/admin/documents/quotes/${item.value.id}`, { onFinish: () => router.visit('/admin/documents/quotes') })
  }
}
...
async function doAction(){
  ...
  const url = action.value==='submit' ? `/admin/documents/quotes/${id}/submit` :
              action.value==='approve' ? `/admin/documents/quotes/${id}/approve` :
              action.value==='lock' ? `/admin/documents/quotes/${id}/lock` :
              `/admin/documents/quotes/${id}/unlock`
