<template>
  <AdminLayout>
    <h1 class="text-xl font-semibold mb-4">แก้ไขใบเสนอราคา</h1>
    <form @submit.prevent="submit" class="space-y-4 text-sm">
      ...
      <div class="flex gap-2">
        ...
        <a :href="`/admin/documents/quotes/${item.id}`" class="px-3 py-1 border rounded">ยกเลิก</a>
      </div>
    </form>
  </AdminLayout>
</template>
...
const form = reactive({
  ...
  customer: {
    name: props.item?.customer?.name ?? '',
    tax_id: props.item?.customer?.tax_id ?? '',
    phone: props.item?.customer?.phone ?? '',
    address: props.item?.customer?.address ?? '',
  },
...
function submit(){
  ...
  router.put(`/admin/documents/quotes/${props.item.id}`, form, { onFinish(){ processing.value = false } })
}
</script>
