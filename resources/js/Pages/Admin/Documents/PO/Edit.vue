<template>
  <AdminLayout>
    <h1 class="text-xl font-semibold mb-4">แก้ไขใบสั่งซื้อ</h1>
    <form @submit.prevent="submit" class="space-y-4 text-sm">
      ...
      <div class="flex gap-2">
        ...
        <a :href="`/admin/documents/po/${item.id}`" class="px-3 py-1 border rounded">ยกเลิก</a>
      </div>
    </form>
  </AdminLayout>
</template>
...
const form = reactive({
  issue_date: props.item?.issue_date ?? new Date().toISOString().slice(0,10),
  due_date: props.item?.due_date ?? '',
  number: props.item?.number ?? '',
  wht_rate_decimal: props.item?.wht_rate_decimal ?? 0,
  vendor: {
    name: props.item?.vendor?.name ?? '',
    tax_id: props.item?.vendor?.tax_id ?? '',
    phone: props.item?.vendor?.phone ?? '',
    address: props.item?.vendor?.address ?? '',
  },
  items: (props.item?.items ?? []).map(i=>({ name: i.name, qty_decimal: i.qty_decimal, unit_price_decimal: i.unit_price_decimal, vat_rate_decimal: i.vat_rate_decimal })) || [ { name: '', qty_decimal: 1, unit_price_decimal: 0, vat_rate_decimal: 0 } ],
  discount_type: props.item?.discount_type ?? 'none',
  discount_value_decimal: props.item?.discount_value_decimal ?? 0,
  deposit_type: props.item?.deposit_type ?? 'none',
  deposit_value_decimal: props.item?.deposit_value_decimal ?? 0,
})
...
function submit(){
  ...
  router.put(`/admin/documents/po/${props.item.id}`, form, { onFinish(){ processing.value = false } })
}
</script>
