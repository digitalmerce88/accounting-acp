  ...
  <Modal :show="showModal" @close="closeModal">
    <div class="p-4 text-sm min-w-[320px]">
      <div class="flex items-center justify-between mb-2">
        <h3 class="text-lg font-semibold">รายละเอียดใบสั่งซื้อ</h3>
        <button class="text-gray-500" @click="closeModal">✕</button>
      </div>
      <div v-if="item" class="flex flex-wrap gap-2 mb-3">
        <a :href="`/admin/documents/po/${item.id}/pdf`" target="_blank" rel="noopener" class="px-2 py-0.5 border rounded">พิมพ์ใบสั่งซื้อ</a>
        <a :href="`/admin/documents/po/${item.id}/edit`" class="px-2 py-0.5 border rounded">แก้ไข</a>
      </div>
...
async function openView(id){
  ...
  loading.value = true
  try{
    const res = await fetch(`/admin/documents/po/${id}`, { headers: { 'Accept':'application/json' } })
