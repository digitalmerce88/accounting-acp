<template>
  ...
  <a :href="`/admin/documents/bills/${item.id}/history`" class="px-3 py-1 border rounded">ประวัติ</a>
  <a :href="`/admin/documents/bills/${item.id}/pdf`" target="_blank" rel="noopener" class="px-3 py-1 border rounded">ดูตัวอย่างบิล</a>
  <a :href="`/admin/documents/bills/${item.id}/pdf?dl=1`" class="px-3 py-1 border rounded">ดาวน์โหลดบิล</a>
  <a :href="`/admin/documents/bills/${item.id}/wht.pdf`" target="_blank" rel="noopener" class="px-3 py-1 border rounded">พิมพ์ใบหัก ณ ที่จ่าย</a>
  <a :href="`/admin/documents/bills/${item.id}/wht.pdf?dl=1`" class="px-3 py-1 border rounded">ดาวน์โหลด WHT</a>
        <a v-if="item.status!=='paid' && !['approved','locked'].includes(item.approval_status)" :href="`/admin/documents/bills/${item.id}/edit`" class="px-3 py-1 border rounded">แก้ไข</a>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 text-sm">
      <div>
        ...
        <div class="font-medium">{{ fmtDMY(item.bill_date) }}</div>
      </div>
      <div>
...
async function pay(){ if(confirm('จ่ายบิลนี้?')) router.post(`/admin/documents/bills/${item.value.id}/pay`, { date: new Date().toISOString().slice(0,10) }) }
async function onDelete(){ if(confirm('ลบเอกสารนี้?')) router.delete(`/admin/documents/bills/${item.value.id}`) }
