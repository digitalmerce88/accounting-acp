<template>
  <AdminLayout>
    <h1 class="text-xl font-semibold mb-4">แก้ไขทรัพย์สิน {{ form.asset_code }}</h1>
    <form @submit.prevent="submit" class="space-y-4 text-sm max-w-2xl">
      ...
      <div class="flex gap-2">
        ...
        <a :href="`/admin/assets/assets/${form.id}`" class="px-3 py-1 border rounded">ยกเลิก</a>
      </div>
    </form>
  </AdminLayout>
</template>
...
const form = reactive({
  id:item.id, asset_code:item.asset_code, name:item.name, category_id:item.category_id, purchase_date:item.purchase_date, purchase_cost_decimal:item.purchase_cost_decimal, salvage_value_decimal:item.salvage_value_decimal, useful_life_months:item.useful_life_months, start_depreciation_date:item.start_depreciation_date
})
...
function submit(){
  ...
  router.put(`/admin/assets/assets/${form.id}`, form, { onFinish(){ processing.value=false } })
}
</script>
