<template>
  <AdminLayout>
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div class="border rounded p-4 bg-white">
        <div class="text-xs text-gray-500">Trial Balance (DR)</div>
        <div class="text-2xl font-semibold">{{ money(metrics.tb_total_dr) }}</div>
      </div>
      <div class="border rounded p-4 bg-white">
        <div class="text-xs text-gray-500">Trial Balance (CR)</div>
    ...
    <div class="mt-6 bg-white border rounded p-4">
      <div class="flex items-center justify-between">
        <h2 class="font-semibold">Recent Journals</h2>
        <Link href="/admin/accounting/journals" class="text-blue-600 text-sm">View all</Link>
      </div>
      <div class="mt-3 overflow-x-auto">
        <table class="min-w-full text-sm">
          ...
          <tbody>
            ...
            <tr v-if="recent_journals.length===0"><td colspan="3" class="p-3 text-center text-gray-500">No recent journals</td></tr>
          </tbody>
        </table>
      </div>
    </div>
