    ...
    <Modal :show="showModal" @close="closeModal">
      <div class="p-4 text-sm min-w-[320px]">
        <div class="flex items-center justify-between mb-2">
          <h3 class="text-lg font-semibold">รายละเอียดรายรับ</h3>
        ...
        <div v-else-if="item">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
            ...
            <div><div class="text-gray-500">วิธีรับ</div><div class="font-medium">{{ item.payment_method }}</div></div>
          </div>
...
async function del(id){
  ...
  async function openView(id){
    ...
    loading.value = true
    try{
      const res = await fetch(`/admin/accounting/income/${id}`, { headers: { 'Accept':'application/json' } })
