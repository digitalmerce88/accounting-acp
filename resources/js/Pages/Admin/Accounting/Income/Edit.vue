    ...
    <form class="mt-4 space-y-3" method="post" :action="`/admin/accounting/income/${item.id}`" enctype="multipart/form-data" @submit.prevent="submit">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          ...
          <div class="mt-1 flex items-center gap-2">
            <select v-model.number="form.customer_id" class="mt-0 border rounded px-2 py-1 w-full">
    ...
    <div v-if="categoryModalOpen" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="bg-white p-4 rounded shadow z-10 w-full max-w-md">
        ...
        <div class="flex justify-end gap-2">
          ...
          <button class="px-3 py-1 bg-blue-600 text-white rounded" @click="createCategory({name:newCategoryName, type:'income'})">สร้าง</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="customerModalOpen" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="bg-white p-4 rounded shadow z-10 w-full max-w-md">
        ...
        <div class="flex justify-end gap-2">
          ...
          <button class="px-3 py-1 bg-blue-600 text-white rounded" @click="createCustomer({name:newCustomerName, phone:newCustomerPhone})">สร้าง</button>
        </div>
      </div>
    </div>
  </AdminLayout>
...
async function createCategory(payload) {
  try {
    ...
    const res = await fetch('/admin/accounting/categories', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' }, body: JSON.stringify(payload) })
    if (!res.ok) throw res
...
async function createCustomer(payload) {
  try {
    const csrf = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''
    const res = await fetch('/admin/documents/customers', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' }, body: JSON.stringify(payload) })
    if (!res.ok) throw res
    const body = await res.json()
    if (body && body.item) {
  ...
  } catch (e) { console.error('createCustomer error', e); alertError('ไม่สามารถสร้างลูกค้าได้') }
}
function submit(){
  busy.value = true
