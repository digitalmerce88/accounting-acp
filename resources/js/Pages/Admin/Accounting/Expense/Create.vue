    ...
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          ...
          <div class="mt-1 flex items-center gap-2">
            <select v-model.number="form.vendor_id" class="border rounded px-2 py-1 w-full">
...
// copy props into local refs so we can append newly created items on the fly
...
async function createVendor(payload) {
  try {
    const csrf = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''
    const res = await fetch('/admin/documents/vendors', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-CSRF-TOKEN': csrf,
    ...
    alertError('ไม่สามารถสร้างผู้ขายได้')
  }
}
function submit(){
  busy.value = true
  ...
  router.post('/admin/accounting/expense', fd, {
    ...
    onSuccess: (page)=>{
      ...
      alertSuccess(msg)
      // controller will redirect to show page
    },
    onError: (resp)=>{
