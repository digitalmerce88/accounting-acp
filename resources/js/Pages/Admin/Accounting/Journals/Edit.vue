<template>
  <AdminLayout>
    <h1 class="text-xl font-semibold">แก้ไขบันทึกรายการ #{{ id }}</h1>
...
async function load(){
  const [accRes, dataRes] = await Promise.all([
    axios.get('/admin/accounting/accounts', { params: { page: 1, q: '' }, headers: { Accept: 'application/json' } }),
    axios.get(`/admin/accounting/journals/${props.id}`, { headers: { Accept: 'application/json' } })
  ])
  accounts.value = accRes.data.data || []
  const e = dataRes.data.entry || {}
  date.value = e.date
  memo.value = e.memo || ''
  lines.value = (dataRes.data.lines || []).map(l=>({ account_id:l.account_id, debit:Number(l.debit||0), credit:Number(l.credit||0) }))
  if (lines.value.length===0) lines.value = [{account_id:null,debit:0,credit:0},{account_id:null,debit:0,credit:0}]
  loading.value = false
}
...
async function submit(){
  error.value = ''
  try {
    await axios.put(`/admin/accounting/journals/${props.id}`, { date: date.value, memo: memo.value, lines: lines.value })
