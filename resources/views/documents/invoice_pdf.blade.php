...
<body>
  ...
  <table style="border:0; margin-bottom:6px;">
    <tr>
      ...
      <td style="border:0; width:40%;" class="box">
        <table class="topgrid">
          <tr><td>เลขที่เอกสาร</td><td class="right">{{ $inv->number ?? $inv->id }}</td></tr>
          <tr><td>วันที่</td><td class="right">{{ $inv->issue_date?->format('Y-m-d') }}</td></tr>
          <tr><td>ครบกำหนด</td><td class="right">{{ $inv->due_date?->format('Y-m-d') }}</td></tr>
        </table>
      </td>
    </tr>
  </table>
  ...
  <table style="border:0; margin-bottom:8px;">
    <tr>
      <td class="box" style="width:60%;">
        <div style="font-weight:700; margin-bottom:4px;">ลูกค้า</div>
  ...
  <table class="totals" style="margin-top:10px;">
    <tr>
      ...
      <td class="right" style="width: 15%; border:1px solid #ccc; padding:6px;">{{ number_format($inv->subtotal,2) }}</td>
    </tr>
    <tr>
      <td class="label">VAT</td>
      <td class="right" style="border:1px solid #ccc; padding:6px;">{{ number_format($inv->vat_decimal,2) }}</td>
    </tr>
    <tr>
      <td class="label" style="font-weight:700;">Total</td>
      <td class="right" style="border:1px solid #ccc; padding:6px; font-weight:700;">{{ number_format($inv->total,2) }}</td>
    </tr>
  </table>
</body>
</html>
