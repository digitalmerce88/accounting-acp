...
<body>
        ...
        <tr>
            <td style="border:0; width:60%;">
                <div style="display:flex; align-items:center; gap:10px;">
                    @if (!empty($company['logo_abs_path']) && file_exists($company['logo_abs_path']))
                        <img class="logo" src="{{ $company['logo_abs_path'] }}" style="width: 25%; height: auto;" />
                    @endif
                </div>
            </td>
            <td style="border:0; width:40%;" class="box">
                <table class="topgrid">
                    <tr>
                        <td>เลขที่เอกสาร</td>
                        <td class="right">{{ $bill->number ?? $bill->id }}</td>
                    </tr>
                    <tr>
                        <td>วันที่</td>
                        <td class="right">{{ $bill->bill_date?->format('d/m/Y') }}</td>
                    </tr>
                    <tr>
                        <td>ครบกำหนด</td>
                        <td class="right">{{ $bill->due_date?->format('d/m/Y') }}</td>
                    </tr>
                </table>
            </td>
        </tr>
    ...
    <table style="border:0; margin-bottom:8px;">
        <tr>
            <td class="box" style="border:0; width:10%;">
                <div class="stack">
            ...
            <td class="box" style="width:45%;">
                <div class="stack">
                    <div style="font-weight:700;">ผู้ขาย/ผู้รับเงิน</div>
                    <div>{{ $bill->vendor->name ?? '-' }}</div>
            <div class="muted">เลขผู้เสียภาษี: {{ $bill->vendor->tax_id ?? '-' }}</div>
    ...
    <table>
        ...
        <tbody>
            ...
            @endforeach
            @for ($i = is_countable($bill->items) ? count($bill->items) : 0; $i < 4; $i++)
                <tr >
                    <td>&nbsp;</td>
                    <td class="right">&nbsp;</td>
                    <td class="right">&nbsp;</td>
    ...
    <table class="totals" style="margin-top:10px;">
        ...
        @if(!empty($bill->discount_type) && $bill->discount_type !== 'none')
        <tr>
            <td class="label">Discount @if($bill->discount_type==='percent') ({{ number_format($bill->discount_value_decimal ?? 0,2) }}%) @endif</td>
            <td class="right" style="border:1px solid #ccc; padding:6px;">-{{ number_format($bill->discount_amount_decimal ?? 0, 2) }}</td>
        </tr>
        @endif
        <tr>
            <td class="label">VAT</td>
            <td class="right" style="border:1px solid #ccc; padding:6px;">{{ number_format($bill->vat_decimal, 2) }}
            </td>
        </tr>
        <tr>
        ...
        @if(!empty($bill->base_total_decimal))
        <tr>
            <td class="label">Base total (เทียบสกุลฐาน)</td>
            <td class="right" style="border:1px solid #ccc; padding:6px;">{{ number_format($bill->base_total_decimal, 2) }}</td>
        </tr>
        @endif
        @if(!empty($bill->deposit_type) && $bill->deposit_type !== 'none')
        <tr>
            <td class="label">Deposit @if($bill->deposit_type==='percent') ({{ number_format($bill->deposit_value_decimal ?? 0,2) }}%) @endif</td>
            <td class="right" style="border:1px solid #ccc; padding:6px;">-{{ number_format($bill->deposit_amount_decimal ?? 0, 2) }}</td>
        </tr>
        <tr>
            <td class="label" style="font-weight:700;">Amount due</td>
            <td class="right" style="border:1px solid #ccc; padding:6px; font-weight:700;">{{ number_format( (float)($bill->total ?? 0) - (float)($bill->deposit_amount_decimal ?? 0), 2) }}</td>
        </tr>
        @endif
    </table>
</body>
